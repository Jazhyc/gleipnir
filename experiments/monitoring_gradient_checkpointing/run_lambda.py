#!/usr/bin/env python3
"""Preflight and benchmark checkpointing-off on one Lambda H100."""
...
from experiments.monitoring_gradient_checkpointing.core import (  # noqa: E402
    JOB_NAME,
    validate_jobs,
...
class Status:
    """Persist the preflight and benchmark state."""
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "results/monitoring_training_throughput/runs/mb1-random/"
    ...
    try:
        ...
        status.update(
            phase="longest_trajectory_preflight",
            preflight="running",
            active_job=preflight_job["job_name"],
        )
        run_training_job(
            preflight_jobs,
            str(preflight_job["job_name"]),
            environment,
            preflight=True,
        )
        ...
        status.update(phase="benchmarking", preflight="passed", active_job=JOB_NAME)
        run_training_job(
            jobs_path,
            JOB_NAME,
            environment,
        ...
        validate_training_metadata(metadata, expected_steps=20)
        status.update(phase="summarizing", active_job=None)
        subprocess.run(
            [
                sys.executable,
                "-m",
                "experiments.monitoring_gradient_checkpointing.summarize",
                "--baseline",
                args.baseline.resolve().as_posix(),
                "--candidate",
                (Path(job["causal_adapter_dir"]) / "training_metadata.json").as_posix(),
                "--output",
