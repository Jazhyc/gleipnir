#!/usr/bin/env python3
"""Run six full-data Muon confirmations on two Lambda H100s."""
...
def relocate_jobs(
    ...
    """Rewrite machine-local paths without changing confirmation identity."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--jobs",
        type=Path,
        default=Path("results/optimizer_full_data_confirmation/jobs.jsonl"),
    )
    parser.add_argument(
        "--lambda-jobs",
        type=Path,
    ...
    parser.add_argument(
        "--status",
        type=Path,
        default=Path("results/optimizer_full_data_confirmation/lambda_status.json"),
    )
    ...
    if args.gpus != 2:
        raise ValueError("the confirmation schedule requires exactly two H100s")
    ...
    try:
        ...
        status.set_phase("full_data_muon_training")
        ...
        status.set_phase("confirmatory_validation")
        subprocess.run(
            [
                sys.executable,
        ...
        subprocess.run(
            [
                sys.executable,
                "experiments/optimizer_full_data_confirmation/summarize.py",
                "--jobs",
                jobs_path.as_posix(),
                "--adamw-references",
                args.adamw_references.resolve().as_posix(),
            ],
            cwd=ROOT,
            check=True,
        )
