#!/usr/bin/env python3
"""Materialize the selective-checkpointing candidate."""
...
DEFAULT_RESULT_DIR = Path("results/monitoring_selective_checkpointing")


def make_job(data_dir: Path, result_dir: Path, selection: Path) -> dict[str, Any]:
    """Adapt the standard AdamW job with only the checkpointing policy changed."""
    base = make_jobs(data_dir, result_dir, selection)[0]
    output = result_dir / "runs" / JOB_NAME
    job = {
        **base,
        "job_name": JOB_NAME,
        "design_role": "selective_checkpointing_candidate",
...
def main() -> None:
    ...
    manifest = {
        "campaign_id": "gleipnir4b-monitoring-selective-checkpointing-v1",
