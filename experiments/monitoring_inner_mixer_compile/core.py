"""Frozen contracts for the eight-step inner-mixer compile screen."""
...
MAX_UNIQUE_GRAPHS = 24
MINIMUM_RELATIVE_IMPROVEMENT = 0.01


def validate_jobs(jobs: list[dict[str, Any]]) -> None:
    """Reject drift from the matched eight-step compile conditions."""
    if [job.get("job_name") for job in jobs] != [CONTROL_NAME, CANDIDATE_NAME]:
        raise ValueError("inner-mixer jobs or order drifted")
    policies = {
    ...
    for job in jobs:
        ...
        for key, value in expected.items():
            if job.get(key) != value:
                raise ValueError(
                    f"inner-mixer job drift for {key}: "
...
def validate_training_metadata(
    ...
    *,
    ...
    if metadata.get("gradient_checkpointing_layer_indices") is not None:
        raise ValueError("explicit checkpointing unexpectedly enabled")
    ...
    expected_disabled_mixers = (
        EXPECTED_CHECKPOINTED_LAYERS if policy == CONTROL_POLICY else []
    )
    ...
    if (
        ...
        raise ValueError("eager linear-mixer boundary drifted")
    if (
        compiled.get("disabled_linear_attention_kernel_layer_indices", [])
        != expected_disabled_kernels
    ):
...
def summarize(jobs: list[dict[str, Any]]) -> dict[str, Any]:
    """Compare segmented inner-mixer compilation with the shell control."""
    validate_jobs(jobs)
