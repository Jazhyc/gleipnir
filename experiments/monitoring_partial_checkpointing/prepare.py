#!/usr/bin/env python3
"""Materialize the matched partial-checkpointing jobs."""
...
DEFAULT_RESULT_DIR = Path("results/monitoring_partial_checkpointing")


def make_jobs(
    data_dir: Path, result_dir: Path, selection: Path
) -> list[dict[str, Any]]:
    """Create the current control and the four-checkpoint candidate."""
    base = make_base_job(data_dir, result_dir, selection)
    jobs = []
    for name, role, checkpoint_policy, checkpoint_indices in (
        (
            CONTROL_NAME,
            "all_linear_checkpoint_control",
            CONTROL_CHECKPOINT_POLICY,
            None,
        ),
        (
            CANDIDATE_NAME,
            "checkpoint_four_linear_candidate",
            CANDIDATE_CHECKPOINT_POLICY,
            CANDIDATE_CHECKPOINTED_LAYERS,
        ),
    ):
        ...
        jobs.append(
            {
                **base,
                "job_name": name,
                "design_role": role,
                "gradient_checkpointing_policy": checkpoint_policy,
                "gradient_checkpointing_layer_indices": checkpoint_indices,
...
def main() -> None:
    ...
    manifest = {
        "campaign_id": "gleipnir4b-monitoring-partial-checkpointing-v1",
