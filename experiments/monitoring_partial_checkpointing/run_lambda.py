#!/usr/bin/env python3
"""Preflight then run the matched partial-checkpointing screen on two H100s."""
...
from experiments.monitoring_partial_checkpointing.core import (  # noqa: E402
...
def run_condition(
    ...
    validate_training_metadata(
        metadata,
        checkpoint_policy=str(job["gradient_checkpointing_policy"]),
    ...
    try:
        ...
        preflight = {
            **candidate,
            ...
            "design_role": "longest_trajectory_partial_checkpoint_preflight",
            "train_rows": 32,
            "max_steps": 1,
        ...
        status.update(
            ...
            preflight_peak_allocated_gib=(
                preflight_metadata["peak_cuda_memory_allocated_bytes"] / 2**30
            ),
        )
        ...
        subprocess.run(
            [
                sys.executable,
                "-m",
                "experiments.monitoring_partial_checkpointing.summarize",
                "--jobs",
                jobs_path.as_posix(),
                "--output",
