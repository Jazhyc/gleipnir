#!/usr/bin/env python3
"""Run the matched monitoring-training throughput screen on two H100s."""
...
from experiments.monitoring_training_throughput.core import (  # noqa: E402
    validate_jobs,
...
class Status:
    """Thread-safe status for the two benchmark lanes."""
...
def run_lane(
    ...
    for job in jobs:
        ...
        validate_training_metadata(json.loads(metadata_path.read_text()), job)
        status.finish(name)
...
def main() -> None:
    ...
    try:
        ...
        lanes = [
            [by_name["mb1-random"], by_name["mb2-length-grouped"]],
            [by_name["mb2-random"]],
        ]
        status.update(phase="benchmarking")
        with ThreadPoolExecutor(max_workers=2) as executor:
            futures = [
                executor.submit(run_lane, lane, gpu, jobs_path, environment, status)
        ...
        subprocess.run(
            [
                sys.executable,
                "-m",
                "experiments.monitoring_training_throughput.summarize",
                "--jobs",
                jobs_path.as_posix(),
                "--output",
