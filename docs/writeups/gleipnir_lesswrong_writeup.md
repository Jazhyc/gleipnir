...
> **TL;DR:** Action-only AI monitors are usually asked to reason through an
> agent trajectory and return an ordinal score that compresses severity and
> confidence into a single value. We instead obtain a confidence score by
> comparing the logits of answer tokens representing benign and problematic
> behavior, improving class separability at lower cost in our matched
> Qwen3.5-9B ablation.
> The advantage of this interface appears to grow with model scale. Distilling
> Kimi K3's binary probabilities into Qwen3.5-4B and 9B models produces the
> strongest low-cost monitors in our comparison. The resulting models dominate
> every open-model point reported on the previous cost--performance frontier.

![OOD monitor cost-performance frontier](../../figures/tool_trajectory_ood_frontier.png)

This work was completed as a 20-hour research project for an application to
Neel Nanda's MATS 12.0 stream.

We next characterize the mismatch between ordinal severity scoring and binary
ranking, describe the logit-distillation setup, and discuss the main limitations
and unresolved ablations.
...
The six OOD sources are CUA-SHADE-Arena, MLE-Sabotage, AgentDojo, IaC, Rogue
Eval, and SHADE-Arena. Together, they span computer use, ML research, tool-use
agents, infrastructure code, inference code, and multi-tool environments.
...
*One-seed soft-distillation scaling compared with approximate values digitized
from Sinha et al.'s Qwen3.5 rationale-SFT curves.*
