...
from experiments.monitoring_linear_shell_compile.prepare import make_jobs
...
def metadata(policy: str, rate: float) -> dict:
    checkpointed = [index for index in range(32) if (index + 1) % 4 != 0]
    ...
    return {
        ...
        "selective_torch_compile": {
            "policy": policy,
            "compiled_layer_indices": (
                EXPECTED_ALL_LAYERS if candidate else EXPECTED_FULL_ATTENTION_LAYERS
            ),
            "disabled_linear_attention_layer_indices": (
                checkpointed if candidate else []
            ),
            "backend": "inductor",
            "mode": "default",
            "dynamic": True,
            "global_torch_compile": False,
            "dynamo_counters": {"stats": {"unique_graphs": 3 if candidate else 1}},
            "canary": None,
        },
        ...
        "optimizer_step_timing": {"steady_mean_seconds": 40.0},
        "peak_cuda_memory_allocated_bytes": 30 * 2**30,
    }
...
def test_jobs_are_matched_eight_step_conditions(tmp_path: Path) -> None:
    ...
    assert (
        ...
        "full_attention_and_linear_shell" in training_command(jobs[1])
    )
...
def test_config_freezes_eight_steps() -> None:
    config = json.loads(
        Path("experiments/monitoring_linear_shell_compile/config.json").read_text()
    )
