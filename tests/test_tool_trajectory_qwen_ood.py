...
def test_frozen_qwen4b_config_uses_full_teacher_ood_contract() -> None:
    config = load_json(
        Path("experiments/tool_trajectory_monitoring/qwen4b_ood_benchmark.json")
    )
    validate_config(config)
    assert config["model"]["id"] == "Qwen/Qwen3.5-4B"
